...
public class MotomanCamera extends PerspectiveCamera implements com.marcowong.motoman.track.logic.ITrackee {

    private void copyMat(com.marcowong.motoman.track.math.Matrix4 src, com.badlogic.gdx.math.Matrix4 dst) {
        System.arraycopy(src.val, 0, dst.val, 0, 16);
    }
    private void copyVec(com.marcowong.motoman.track.math.Vector3 src, com.badlogic.gdx.math.Vector3 dst) {
        dst.set(src.x, src.y, src.z);
    }
    private void copyQua(com.marcowong.motoman.track.math.Quaternion src, com.badlogic.gdx.math.Quaternion dst) {
        dst.set(src.x, src.y, src.z, src.w);
    }
    private com.badlogic.gdx.math.Matrix4 getGdxMat(com.marcowong.motoman.track.math.Matrix4 src) {
        com.badlogic.gdx.math.Matrix4 dst = new com.badlogic.gdx.math.Matrix4();
        copyMat(src, dst);
        return dst;
    }
	...
	@Override
	public void getTrackeePos(com.marcowong.motoman.track.math.Vector3 vec) {
	...
	public void followMotorcycle(float delta) {
		if (!motorcycle.logic.state.isCrashed) {
			...
			float s = 0;
			float engineOutput = motorcycle.logic.getEngineOutputPercentage();
			...
			if (state.vanishingPointNeedReset) {
				...
				state.vanishingPoint.set(0, 0, 10).mul(getGdxMat(motorcycle.logic.state.pos));
				...
				if (stepFullLen > 0) {
					float step1 = motorcycle.logic.state.bikeVelo.len() * 2;
			...
			copyVec(motorcycle.logic.state.pos.getTranslation(new com.marcowong.motoman.track.math.Vector3()), tmpVec);
			tmpVec.y = 0;
			tmpVec2.set(0, 1, 0);
			...
			copyQua(motorcycle.logic.state.pos.getRotation(new com.marcowong.motoman.track.math.Quaternion()), tmpQua);
			tmpQua.slerp(tmpQua2, state.vanishingPointLookingFactor).nor();
			tmpVec.set(state.motorcycleCounterSteeringShift, 5, 0).mul(getGdxMat(motorcycle.logic.state.pos));
			tmpMat2.idt().set(tmpQua).trn(tmpVec);
			tmpVec.set(0, 0, -(12  + state.distance)).add(tmpVec5).mul(tmpMat2);
			tmpVec2.set(state.motorcycleCounterSteeringShift, 5, 0).mul(getGdxMat(motorcycle.logic.state.pos));
			...
			motorcycle.logic.getTrackeePos(new com.marcowong.motoman.track.math.Vector3());
			// Need to fix getTrackeePos
			{
			com.marcowong.motoman.track.math.Vector3 tv = new com.marcowong.motoman.track.math.Vector3();
			motorcycle.rider.logic.getTrackeePos(tv);
			copyVec(tv, tmpVec2);
		}
	...
	public void alignCameraWithDevice() {
		...
		tmpMat.rotate(0, 0, 1, -deviceRot +
				(motorcycle.logic.state.isCrashed ? 0 : state.shakeRot * state.shakeness * shakeRotFactor));
