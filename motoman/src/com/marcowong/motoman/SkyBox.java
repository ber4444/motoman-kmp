...
public class SkyBox {
	...
	private static ModelData modelPreBloomed;
	...
	private static void preBloomModel() {
		...
		SubMeshData[] bloomingMeshes = model.subMeshes;
		SubMeshData[] bloomedMeshes = modelPreBloomed.subMeshes;
		for (int k = 0; k < bloomedMeshes.length; ++k) {
			if (bloomingMeshes[k].material != null && bloomingMeshes[k].material.diffuseTexture != null &&
				bloomedMeshes[k].material != null && bloomedMeshes[k].material.diffuseTexture != null) {
				Texture tattrBlooming = bloomingMeshes[k].material.diffuseTexture;
				
				TextureData tdata = tattrBlooming.getTextureData();
				tdata.prepare();
				...
				Texture oldTexture = bloomedMeshes[k].material.diffuseTexture;
				if (oldTexture != null && !oldTexture.isManaged()) oldTexture.dispose();
				
				Texture newTexture = new Texture(m2, false);
				newTexture.setFilter(tattrBlooming.getMinFilter(), tattrBlooming.getMagFilter());
				newTexture.setWrap(tattrBlooming.getUWrap(), tattrBlooming.getVWrap());
				bloomedMeshes[k].material.diffuseTexture = newTexture;
				m2.dispose();
			}
		}
	}
