...
public class MeshOptimized {
	...
	private CreateOptimizedMeshResult createOptimizedMesh(Set<MeshData> meshes, int copies) {
		...
		for (MeshData mesh : meshes) {
			int vertice0VSize = 3 + (mesh.hasNorms ? 3 : 0) + (mesh.hasUVs ? 2 : 0) + (mesh.hasSkeleton ? 1 : 0);
			int numVertices = mesh.vertices.length / vertice0VSize;
			float[] vertice0 = mesh.vertices;
			float[] vertice = new float[numVertices * 9];
			for (int i = 0; i < numVertices; ++i) {
				int offsetPos = 0;
				vertice[i * 9 + 0] = vertice0[i * vertice0VSize + offsetPos + 0];
				vertice[i * 9 + 1] = vertice0[i * vertice0VSize + offsetPos + 1];
				vertice[i * 9 + 2] = vertice0[i * vertice0VSize + offsetPos + 2];
				
				if (mesh.hasNorms) {
					int offsetNor = 3;
					vertice[i * 9 + 3] = vertice0[i * vertice0VSize + offsetNor + 0];
					vertice[i * 9 + 4] = vertice0[i * vertice0VSize + offsetNor + 1];
					vertice[i * 9 + 5] = vertice0[i * vertice0VSize + offsetNor + 2];
				}
				
				if (mesh.hasUVs) {
					int offsetTex = 3 + (mesh.hasNorms ? 3 : 0);
					vertice[i * 9 + 6] = vertice0[i * vertice0VSize + offsetTex + 0];
					vertice[i * 9 + 7] = vertice0[i * vertice0VSize + offsetTex + 1];
				}
				
				if (mesh.hasSkeleton) {
					int offsetSke = 3 + (mesh.hasNorms ? 3 : 0) + (mesh.hasUVs ? 2 : 0);
					vertice[i * 9 + 8] = vertice0[i * vertice0VSize + offsetSke + 0];
					if (vertice[i * 9 + 8] > maxSkeId)
						maxSkeId = (int)vertice[i * 9 + 8];
				}
			}
			short[] indice = new short[mesh.indices != null ? mesh.indices.length : 0];
			if (mesh.indices != null) System.arraycopy(mesh.indices, 0, indice, 0, indice.length);
			for (int i = 0; i < indice.length; ++i)
				indice[i] += nVerticeT;
			nVerticeT += numVertices;
	...
	public IMeshContext add(ModelData model) {
		return add(model, 1);
	}
	
	public IMeshContext add(ModelData model, int copies) {
		SubMeshData[] meshes = model.subMeshes;
		
		Map<MaterialData, Set<MeshData>> matMap = new Hashtable<MaterialData, Set<MeshData>>();
		for (SubMeshData mesh : meshes) {
			Set<MeshData> meshSet = matMap.get(mesh.material);
			if (meshSet == null) {
				meshSet = new HashSet<MeshData>();
		...
		for (Entry<MaterialData, Set<MeshData>> kvp : matMap.entrySet()) {
	...
	public IMeshContext add(MeshData mesh, int primitiveType) {
		MeshContext meshContext = new MeshContext();
		meshContext.renderContexts = new ArrayList<RenderContext>();
		meshContext.primitiveType = primitiveType;
		meshContext.copies = 1;
		Set<MeshData> meshes = new HashSet<MeshData>();
		meshes.add(mesh);
