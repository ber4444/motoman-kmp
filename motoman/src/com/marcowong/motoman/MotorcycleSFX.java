...
public class MotorcycleSFX {
	...
	public void update(float delta) {
		if (motorcycle.logic.state.isCrashed) {
			...
			float engineOutputRaw = motorcycle.logic.getRawEngineAndBrakeMeter();
			...
			if (motorcycle.logic.state.isStandBy) {
				...
				float leanFactor = (90 - Math.abs(motorcycle.logic.state.leanAngle) * 0.333f) / 90;
				...
				if (lastEngineOutput > engineOutput &&
					Math.abs(motorcycle.logic.state.leanAngle) <= 30)
			...
			float engineMeter = motorcycle.logic.getEngineAndBrakeMeter();
