...
public class ObjLoaderEx {
	...
	public ModelData loadObj (FileHandle file) {
	...
	public ModelData loadObj (FileHandle file, boolean flipV) {
	 ...
	 * @param textureDir
	 * @param flipV whether to flip the v texture coordinate (Blender, Wings3D, et al) */
	public ModelData loadObj (FileHandle file, FileHandle textureDir, boolean flipV) {
		String line;
		String[] tokens;
		char firstChar;
		...
		model.subMeshes = new SubMeshData[numGroups];
		...
		for (int g = 0; g < numGroups; g++) {
			...
			mesh.indices = (numIndices > 0) ? finalIndices : null;
			...
			subMesh.mesh = mesh;
			subMesh.primitiveType = GL20.GL_TRIANGLES;
...
class MtlLoader {
	private ArrayList<MaterialData> materials = new ArrayList<MaterialData>();
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				if (tokens[0].length() == 0) {
					...
					MaterialData mat = new MaterialData(curMatName);
					mat.diffuseTexture = texture;
					mat.ambientColor = ambcolor;
					mat.diffuseColor = difcolor;
					mat.specularColor = speccolor;
					mat.shininessColor = shininesscolor;
					materials.add(mat);
	...
	public MaterialData getMaterial (String name) {
		...
		return new MaterialData("default");
	}
}
