...
public class MotomanGameScreen implements Screen {
	...
	private static MeshData frameBufferMesh;
	...
	public static void initResource() {
        ...
        frameBufferMesh = new MeshData();
        frameBufferMesh.hasUVs = true;
        frameBufferMesh.vertices = new float[] { 1, 1, 0, 1, 1, -1, 1, 0, 0, 1, 1, -1, 0, 1, 0, -1, -1, 0, 0, 0};
        frameBufferMesh.indices = new short[] { 0, 1, 2, 3 };
	...
	private com.marcowong.motoman.track.logic.IMotorcycleInputMeters motorcycleInputMeters;
	...
	public MotomanGameScreen(TrackData trackData) {
		...
		{ com.marcowong.motoman.track.math.Matrix4 m = track.logic.getStartSpawnPosition(); System.arraycopy(m.val, 0, motorcycle.logic.state.pos.val, 0, 16); }
	...
	public void render(float oDelta) {
		...
		if (gameUpdating) {
			...
			while (true) {
				...
				TrackSegment motorcycleTrackSegBeforeUpdate = motorcycle.logic.getLastTrackSegment();
				...
				if (isPersistUpdateStep) {
					...
					if (!motorcycle.logic.state.isCrashed &&
						!motorcycle.logic.state.isStandBy &&
						motorcycle.logic.getLastTrackSegment() != motorcycleTrackSegBeforeUpdate)
							trackDirectionNotice = track.logic.getTrackeeDirectionNotice(motorcycle.logic);
	...
	//			camera.update(false);
		...
		track.getTrackeeTrackMapLoc(motorcycle.logic, tmpVec);
		trackLocImg.setBounds(
				...
				Gdx.graphics.getHeight() * 0.02f);
		if (!motorcycle.logic.state.isCrashed) motorcycleTrackFinishedPercentage = (int)Math.round(track.logic.getTrackeeTrackPercentage(motorcycle.logic) * 100);
	...
	private void runGameRules(float delta) {
		...
		if (!motorcycleCrashed &&
			motorcycle.logic.state.isCrashed) {
			motorcycleCrashed = true;
			{ System.arraycopy(motorcycle.logic.state.pos.val, 0, motorcycleCrashedPos.val, 0, 16); }
			motorcycleCrashedTimeRemaining = 2;
		}
		if (motorcycleCrashed &&
			motorcycleCrashedTimeRemaining <= 0) {
			motorcycleCrashed = false;
			motorcycle.logic.reset();
			{ System.arraycopy(motorcycleCrashedPos.val, 0, motorcycle.logic.state.pos.val, 0, 16); }
			{ com.marcowong.motoman.track.math.Matrix4 m = track.logic.getSpawnPosition(motorcycle.logic); System.arraycopy(m.val, 0, motorcycle.logic.state.pos.val, 0, 16); }
			resetMotionBlurData();
		}
		if (!motorcycleCrashed &&
			!motorcycleStandBy &&
			!motorcycleFinished &&
			track.logic.isInTrackEnd(motorcycle.logic)) {
			motorcycleFinished = true;
			motorcycleFinishedTimeRemaining = 2;
			motorcycle.logic.standBy();
		}
		if (motorcycleFinished &&
			motorcycleFinishedTimeRemaining <= 0) {
			motorcycleFinished = false;
//			motorcycle.logic.reset();
//			motorcycle.pos.set(track.logic.getStartSpawnPosition());
//			rider.setStrength(1);
			gameFinished = true;
			gameWined = true;
		}
